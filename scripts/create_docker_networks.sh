...
usage() {
  cat <<'USAGE'
Usage: ./scripts/create_docker_networks.sh [--recreate]

Creates the two Docker ipvlan networks used by the radio VLANs:
...
require_parent() {
  ...
  if ! ip link show "${parent}" >/dev/null 2>&1; then
    ...
    echo "Run sudo ./scripts/setup_parent_interfaces.sh <parent_interface> first." >&2
    exit 1
  fi
}
...
validate_network() {
  ...
  if [[ "${driver}" == "ipvlan" && "${parent}" == "${expected_parent}" && "${subnet}" == "${expected_subnet}" ]]; then
  ...
  echo "  found:    driver=${driver} parent=${parent} subnet=${subnet}" >&2
  echo "  expected: driver=ipvlan parent=${expected_parent} subnet=${expected_subnet}" >&2
  echo "Run this after stopping attached containers: ./scripts/create_docker_networks.sh --recreate" >&2
  exit 1
}
...
create_network bridge_lo 10.0.10.0/24 10.0.10.200 eth_umrt.10
create_network bridge_hi 10.0.20.0/24 10.0.20.200 eth_umrt.20
